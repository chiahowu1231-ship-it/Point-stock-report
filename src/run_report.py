...
DEFAULT_DAYS = 5  # 固定 5 日區間（可用環境變數 DAYS 覆蓋）
HEAD_BROKER = "9600"  # 你的 GUI 使用的 broker_head
...
def parse_table(html: str) -> dict:
    """
    解析嘉實頁面表格，回傳:
    { stock_id: {'name':..., 'buy':..., 'sell':..., 'net': int } }
    """
...
def build_report(days: int):
    """
    主流程：抓各傳奇分點資料 → 計算均價/乖離 → 組成 DataFrame
    回傳：df(Report), fail_df(Failures), summary(dict)
    """
    ...
    for broker_id, broker_name in LEGEND_BROKERS.items():
        ...
        try:
            ...
            for sid, info in qty_map.items():
                ...
                net_qty = info["net"]
                net_amt = amt_map[sid]["net"]  # 仟元（來源頁面）
...
def export_excel(df: pd.DataFrame, fail_df: pd.DataFrame, xlsx_path: str):
    """輸出 Excel：Report + Failures，並對乖離率(J欄)做條件式上色。"""
    ...
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        ...
        if last_row >= 2:
            ...
            # 乖離率是字串 like "3.25%" → 用 SUBSTITUTE 去掉 % 再 VALUE
...
def export_pdf(df: pd.DataFrame, pdf_path: str, summary: dict):
    """
    方案 B1：嵌入 TrueType 字型避免亂碼（NotoSansTC-Regular.ttf）。
    需要 repo 根目錄存在 fonts/NotoSansTC-Regular.ttf。
    """
    ...
    if not os.path.exists(font_path):
        raise FileNotFoundError(
            f"找不到字型檔：{font_path}。請將 NotoSansTC-Regular.ttf 放到 repo 的 fonts/ 目錄下。"
        )
    ...
    if df is None or df.empty:
        header = ["日期", "代碼", "名稱", "大戶", "買進", "賣出", "淨超", "區間均價", "現價", "乖離率"]
        data = [header]
    else:
...
def main():
    ...
    # 若有任何分點失敗，讓該 step fail（workflow 後續 if: always() 仍會寄信/上傳）
